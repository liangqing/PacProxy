...
, begin4: function(data) {
    ...
    Logger.debug('client using version 4')
    if(CMD === 0x1) {
      this.request(address)
    } else {
      Logger.error('unsupport CMD('+CMD+') in version 4')
      this.end()
    }
  }
...
, begin5: function(data) {
    var nMethod = data.readUInt8(1)
      , socket = this.socket
    Logger.debug('client using version 5')
    Logger.debug('nMethod:'+nMethod)
    for(;nMethod--;) {
      Logger.debug('method:'+data.readUInt8(nMethod+2))
    }
    socket.write(new Buffer([0x05, 0x00]), function() {
      Logger.debug('reply version and methods');
    })
    this.state = 'request5'
  }
...
, request5: function(data) {
    ...
    Logger.debug('RSV:'+data.readUInt8(2))
    if(CMD === 0x1) {
      this.request(address)
    } else {
      Logger.error('unsupport CMD('+CMD+') in version 5')
      this.end()
    }
  }
...
, pipe: function(remote) {
    Logger.debug('piping data: '+this.socket.remoteAddress+':'+this.socket.remotePort+'<->'+remote.remoteAddress+':'+remote.remotePort)
...
, connected: function(data) {
    ...
    if(reply === 0) {
      ...
      Logger.error('request failed: reply code is ' + reply)
